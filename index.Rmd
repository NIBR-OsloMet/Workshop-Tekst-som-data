...
# Intro
Veiledet maskinlæring (_supervised_) skiller seg fra ikke-veiledet læring (_unsupervised_) ved at man har tilgang til et annotert datasett med både input og output, ofte kalt for treningsdata. Treningsdata blir gjerne kodet av forskerne i forberedelsesfasen, før den blir brukt videre til å trene og teste maskinlæringsmodeller. Målet gjennom treningen av veiledete modeller er å kartlegge forholdet mellom input og output så nøyaktig at den kan brukes til å klassifisere ny, usett data, eller til å forutsi utfall og trender. 

Grunnen til at det kalles _veiledet_ maskinlæring er fordi deler av prosessen krever aktivt tilsyn og veiledning av mennesker, i motsetning til ikke-veiledet læring hvor menneskets rolle er minimal. 
...
Veiledet læring brukes til å klassifisere usett data i forhåndsdefinerte kategorier, eller til å forutsi trender og fretidige endringer med en prediktiv modell, og blir gjerne delt inn i to hovedgrupper for problemløsning: klassifikasjon og regresjon. 

* **Klassifikasjon**: Innen klassifikasjonsproblemer trenes algoritmer til å fordele data inn i spesifikke kategorier. 
...
* **Regresjon**: Innen regresjonsproblemer trener man algoritmer til å forstå forholdet mellom avhengige og uavhengige variabler, og for å predikere nummeriske verdier eller forutsi trender. 
...
Dette dokumentet skisserer et hands-on eksempel på hvordan fremgangsmåten for å løse et binært klassifikasjonsproblem kan være. Det er likevel viktig å påpeke at prosessen som skisseres her er svært forenklet. Målet er å gi et lite innblikk i hvordan veiledet maskinlæring kan bli benyttet for tekstanalyse, men dokumentet gir ingen dyp teknisk innføring i veiledet læring.
...
I boka [Text as Data av Grimmer, Roberts og Stewart](https://www.akademika.no/pedagogikk-og-samfunnsvitenskap/sosiologi/text-data/9780691207551) gir forfatterne en grundig innføring i hva som kjennetegner god treningsdata og en god kodebok gjennom kapittel 19. Uten å gå altfor grundig til verks kan et par begreper trekkes frem som særlig relevante:
...
For at algoritmen skal kunne bruke tekstdataen effektivt må dataen bearbeides på en måte som er ganske spesifikk for tekstdata. I dette tilfellet vil jeg unneste tokens, filtere vekk alle symboler som ikke er latinske bokstaver og fjerne lite brukte ord. Det er ved dette steget du også evt ville ha fjernet stoppord.
...
tweets_recipe <- recipe(label~texts, data=train) |>
  step_tokenize(texts) |>
  step_tokenfilter(texts, min_times = 3, max_tokens = 500) |>
  step_tfidf(texts) |>
  step_stopwords(custom_stopword_source = stopwords, keep = FALSE) |>
  step_normalize(all_predictors()) |>
  prep()

new_train <- bake(tweets_recipe, new_data = train)
...
# Evaluering
Vi har allerede trent en klassifikasjonmodell som har en accuracy på 0.94 for treningsdataen. Men hvor godt presterer den på testdataen, og hvordan kan man egentlig evaluere prestasjonen til modellen?
...
**Overfitting** innebærer kort fortalt at modellen gir nøyaktige prediksjoner på treningsdataen, men ikke på testdataen. Denne skjer typisk når modellen er for kompleks, du har trent den så mye at den gjenkjenner all støy og deltajer i treningsdataen som relevante mønstre. Denne kompleksiteten vil gi veldig nøyaktige predisjoner på treningsdataen, men modellen vil ikke kunne generaliseres til ny data. 
**Underfitting** innebærer at en modell er for enkel til å kunne fange opp relevant kompleksitet i dataen, og modellen vil gi dårlige resultater både på trenignsdata og testdata. 
